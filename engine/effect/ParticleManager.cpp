...
void ParticleManager::Initialize(DX12Basic* dx12)
{
	dxCommon_ = dx12;
...
void ParticleManager::Update()
{
	...
	Matrix4x4 cameraMatrix = Mat4x4::MakeAffine({ 1.0f,1.0f,1.0f }, camera->GetRotate(), camera->GetTranslate());

	Matrix4x4 viewMatrix = Mat4x4::Inverse(cameraMatrix);
	Matrix4x4 projectionMatrix = Mat4x4::MakePerspective(0.45f, float(kWindowWidth) / float(kWindowHeight), 0.1f, 100.0f);
...
{
	//==========頂点リソース生成==========//
	vertexResource = dxCommon_->MakeBufferResource(sizeof(VertexData) * modelData.vertices.size());
...
void ParticleManager::CreateParticleGroup(const std::string name, const std::string textureFilePath, BlendMode blendMode)
{
	...
	newGroup.instancingResource =
		dxCommon_->MakeBufferResource(sizeof(ParticleForGPU) * kNumMaxInstance);
	newGroup.instancingResource->Map(0, nullptr, reinterpret_cast<void**>(&newGroup.instancingDataPtr));
	for (uint32_t index = 0; index < kNumMaxInstance; ++index)
	{
		newGroup.instancingDataPtr[index].WVP = Mat4x4::MakeIdentity();
		newGroup.instancingDataPtr[index].World = Mat4x4::MakeIdentity();
...
{
	// マテリアル用のリソースを作成
	materialResource = dxCommon_->MakeBufferResource(sizeof(Material));
	...
	materialData->uvTransform = Mat4x4::MakeIdentity();
}


void ParticleManager::InstancingMaxResource()
{
	Microsoft::WRL::ComPtr<ID3D12Resource> instancingResource2 = dxCommon_->MakeBufferResource(sizeof(ParticleForGPU) * kNumMaxInstance);
	ParticleForGPU* instancingData2 = nullptr;
	instancingResource2->Map(0, nullptr, reinterpret_cast<void**>(&instancingData2));
	for (uint32_t index = 0; index < kNumMaxInstance; ++index)
	{
		instancingData2[index].WVP = Mat4x4::MakeIdentity();
		instancingData2[index].World = Mat4x4::MakeIdentity();
...
void ParticleManager::Emit(const std::string name, const Vector3& position, uint32_t count)
{
	...
	for (uint32_t i = 0; i < count; ++i) {
		...
		group.particleList.push_back(MakeNewParticle(randomEngine_, position));
	}
}
...
Microsoft::WRL::ComPtr<ID3D12PipelineState> ParticleManager::PSO()
{
	...
	HRESULT hr = dxCommon_->GetDevice()->CreateGraphicsPipelineState(&graphicsPipelineStateDesc, IID_PPV_ARGS(&pipelineState));
